...
if uploaded_file is not None and len(train) > 0 and len(test) > 0:
    ...
    if st.button("Tạo dự đoán"):
        ...
        fig, ax = plt.subplots()
        ax.plot(predictions_df.index, predictions_df[f"{target_column}_Dự đoán"])
        st.pyplot(fig)


# Bước 5: Dự đoán với điểm bắt đầu và kết thúc xác định
st.header("Bước 5: Dự đoán với Điểm Bắt đầu và Kết thúc")
